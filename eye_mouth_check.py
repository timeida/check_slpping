...
#----------------------start here------------------------(직접 구현하였으나, 기본 틀과 원리는 위의 깃허브 코드를 참고하여 설계함)
def mouth_is_close(mouth_ratio, mouth_open_counter, Open_Start, mouth_open_total,WarningSound,frame,mouth_at_tresh,frame_num):
    
    # mouth_at_tresh 입 벌린 정도 야간=(주간)*0.8
    ...
    if mouth_ratio > mouth_at_tresh:
        ...
        if Open_Start > frame_num: #값은 나중에 조절하자
            
            WarningSound.play(loops=1)
            if WarningSound.play(loops=1):
                print("졸음 경고 : 하품 기준치 이상")
