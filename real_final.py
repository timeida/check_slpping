...
from set_mode import set_night_mode,reset_camera_settings   
from bright_check import is_night_mode
    ...
    while True:
        ...
        if ret: #영상 성공적으로 읽음
            ...
            if check_night_mode_counter == night_mode_check_interval: #루프 50번 돌때마다 모드 체크해서 낮,밤 확인
                ...
                check_night_mode_counter = 0  # Reset the counter after checking night mode
        ...
        if not(mode): 
            ...
            if len(faces):
                for x, y, w, h in faces:
                    ...
                    if not face_detected:  # face_detected가 False일 때만 출력
                        start = 1
                        print("운전자 탑승: 졸음운전 감지를 시작합니다. by--Open cv")
            ...
            for rect in rects:
                ...
                mouth_open_total, mouth_open_counter, Open_Start = mouth_is_close(mouth_ratio, mouth_open_counter,Open_Start,mouth_open_total,WarningSound,frame,1.0)
                
                #위 함수 2개(진우), 눈 감은지, 하품하는지 판단하고, 카운트해줌, 값들 계속 루프돌면서 업데이트함

                cv2.putText(frame, "Blinks " + str(eye_blink_total) , (10, 30), cv2.FONT_HERSHEY_SIMPLEX, 0.7, (0, 0, 255), 2)
                cv2.putText(frame, "EAR: " + str(ear), (300, 30), cv2.FONT_HERSHEY_SIMPLEX, 0.7, (0, 0, 255), 2)
                cv2.putText(frame, "MOUTH_RATIO: " + str(mouth_ratio), (300, 10), cv2.FONT_HERSHEY_SIMPLEX, 0.7, (0, 0, 255), 2)
                cv2.putText(frame, "Open " + str(mouth_open_total) , (10, 10), cv2.FONT_HERSHEY_SIMPLEX, 0.7, (0, 0, 255), 2)
                #출력 텍스트 만드는 부분(진우)
        ...
        #     print("야간모드일때만 출력돼야해")
            ...
            if start == 1:#운전자 탑승 확인 되면 졸음 여부 판단 시작
                ...
                if elapsed_time > 2.5:
                    ...
                    print("운전자 졸음 감지: 운전자 눈 감고있음  by--Open cv")
            ...
            for rect in rects:
                ...
                mouth_open_total, mouth_open_counter, Open_Start = mouth_is_close(mouth_ratio, mouth_open_counter,Open_Start,mouth_open_total,WarningSound,frame,0.8)
